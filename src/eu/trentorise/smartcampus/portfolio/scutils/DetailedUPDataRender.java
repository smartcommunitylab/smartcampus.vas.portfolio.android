...
/**
 * Render for detailed user produced data fragment
 * 
 * @author Simone Casagranda
 * 
 */
public class DetailedUPDataRender {
	...
	public static void render(Holder holder, UserProducedData data) {
		...
		if (Constants.RAW.equalsIgnoreCase(data.type)) {
			...
			holder.description.setText(data.content);
		...
		} else if (Constants.SIMPLE_DESC.equalsIgnoreCase(data.type)) {
			// We hide all views
			holder.title.setVisibility(View.VISIBLE);
			holder.subtitle.setVisibility(View.VISIBLE);
			holder.description.setVisibility(View.VISIBLE);
